...
class GoogleDriveProcessor:
    ...
    def download_file(
        ...
        """Download a file from Google Drive to temporary storage and return path + bytes."""
        if not self.api_key:
            return None
        ...
        try:
            ...
            file_bytes.seek(0)  # Reset to start for reading
            ...
            # Reset BytesIO for return
            file_bytes.seek(0)
            return temp_file, file_bytes.getvalue()
    ...
    async def process_new_files(self) -> List[Dict]:
        ...
        for file_info in new_files:
            ...
            try:
                ...
                # Download file (now returns path and bytes)
                download_result = self.download_file(file_id, file_name)
                if not download_result:
                    continue
                temp_file, file_bytes = download_result
                ...
                results.append(
                    {
                        ...
                        "data": structured,
                        "bytes": file_bytes,  # Use the captured bytes
                ...
                logger.info(
                    f"✅ Successfully processed {file_name} -> Candidate ID {candidate_id}"
                )
