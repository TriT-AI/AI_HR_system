            ...
            if st.button("Process Now", type="primary"):
                ...
                with st.spinner("Processing files..."):
                    for idx, file in enumerate(uploaded_files, 1):
                        utils.display_progress(idx, len(uploaded_files), f"Processing: {file.name}")
                        path, bytes_data = utils.save_upload_and_cache_bytes(file)
                        try:
                            structured = utils.run_async(st.session_state.processor.process_resume(str(path)))
                ...
                for res in results:
                    with st.expander(f"{res['name']} (ID: {res['candidate_id']})"):
                        ...
                        with col2:
                            ...
                            st.json(res["data"])
                            utils.export_json(res["data"], "⬇️ Export JSON", f"{res['candidate_id']}.json")
        else:
        ...
        if not stats["configured"]:
            ...
            if st.button("Sync Now"):
                with st.spinner("Syncing..."):
                    new_files = utils.run_async(st.session_state.drive_processor.process_new_files())
                    if new_files:
